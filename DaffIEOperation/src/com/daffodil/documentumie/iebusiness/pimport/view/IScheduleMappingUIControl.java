...
import com.daffodil.documentumie.iebusiness.IEMessageUtility;
...
public class IScheduleMappingUIControl extends AbstractUIControl
{
	...
	XMLMetadataReader mtrdr=new XMLMetadataReader(config.getDocumentumHome().replace("\\", "/")+"/DaffIE/","IScheduleConfigBean");
	...
	public IScheduleMappingUIControl() 
	{
		initComponents();
		initlizeUI();
	}
	...
	private String getSecondryXmlFile() 
	{
		...
		String fileName = fileDir+"/"+"ScheduleImportConfigBean.xml";
	...
	protected void installListener() 
	{
		remove_JButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				IEMessageUtility ie=new IEMessageUtility(fieldMapping_JTable);
				int confirm=ie.showConfirmationMessageDialog("Are you sure you want to delete?");
				if(confirm==0)
				{
	...
	private void getMetadataFromScheduleFile()
	{
	...
	private void getAttributefromRepository() 
	{
		...
		try 
		{
			...
			{
				 if(((String)attributes.get(i)).startsWith("r_") || ((String)attributes.get(i)).startsWith("i_"))
				 {
					 continue; 
				 }
	...
	private void showFieldInTable() 
	{
		...
		/**
		 * 1.rows count
		 * 2.values of the rows
		 * 
		 */
		List list=new ArrayList();
		System.out.println("Total no of rows"+mtrdr.getRowsCountXML());
		
		list=mtrdr.getRowsXML();
		for (int i = 0; i < mtrdr.getRowsCountXML(); i++)
		{
...
//		 attributeMap.put("Image_Path", "Image_Path");
	...
	private HashMap getTableColumns()
	{
		...
		//XMLMetadataReader mtrdr=new XMLMetadataReader("E:\\Harsh Crop Project\\Schedule Task\\DaffIE\\","IScheduleConfigBean");
		HashMap hmap=new HashMap();
		
		hmap=mtrdr.getColumnXml();
		return hmap;
	}
